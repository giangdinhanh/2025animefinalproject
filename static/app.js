...
function initForecast(){
  ...
  if(typeof Chart==='undefined'){
    if(status)status.textContent='Chart.js could not load.';
    return;
  }
  const data=window.ANIBOMB_FORECAST_DATA;
  if(!data || !Array.isArray(data.historical) || !Array.isArray(data.forecast)){
    if(status)status.textContent='Forecast data could not be initialized. Confirm static/forecast-data.js is present.';
    return;
  }

  const hist=data.historical;
  const fc=data.forecast;
  const years=[...hist.map(d=>d.year), ...fc.map(d=>d.year)];
  const histValues=[...hist.map(d=>d.favorites), ...Array(fc.length).fill(null)];

  // Start the forecast line at the last historical point so the transition is visually continuous.
  const forecastValues=[
    ...Array(Math.max(0,hist.length-1)).fill(null),
    hist[hist.length-1].favorites,
    ...fc.map(d=>Math.round(d.favorites))
  ];
  ...
  forecastChart=new Chart(canvas,{
    ...
    data:{
      ...
      datasets:[
        ...
        {
          label:'Forecast Favorites',
          data:forecastValues,
          borderColor:'#7f79f2',
          backgroundColor:'rgba(127,121,242,.12)',
          borderWidth:3,
          borderDash:[8,6],
          pointRadius:4,
          pointHoverRadius:8,
          pointBackgroundColor:'#7f79f2',
          tension:.28,
          fill:false,
          spanGaps:false
        }
    ...
    options:{
      ...
      plugins:{
        legend:{
          display:true,
          position:'top',
          labels:{usePointStyle:true,boxWidth:9,padding:18}
        },
        tooltip:{
          enabled:true,
      ...
      scales:{
        ...
        y:{
          beginAtZero:true,
          grid:{color:'rgba(17,17,17,.06)'},
          border:{display:false},
          ticks:{callback:v=>fmtCompact(v)},
          title:{display:true,text:'Total Favorites',color:'#66627a',font:{weight:'700'}}
        }
      }
    }
  });
  ...
  if(status){
    const first=fc[0], last=fc[fc.length-1];
    status.textContent=`Workbook forecast loaded: ${first.year} = ${Math.round(first.favorites).toLocaleString()} Favorites → ${last.year} = ${Math.round(last.favorites).toLocaleString()} Favorites. Seasonality = ${data.meta.seasonality}.`;
  }
}
